---
description: Scaffold a new MDX daily briefing with valid frontmatter
argument-hint: <topic or working title>
allowed-tools: Bash(date:*), Write, Read
---

Create a new daily briefing for Non Slop AI about: **$ARGUMENTS**
...
3. Write the file to `content/briefings/<slug>.mdx` with this exact frontmatter
   shape (validated by the Zod schema in `src/lib/briefings.ts` — the build fails if
   it drifts):
...
4. Below the frontmatter, write a short, honest outline as MDX — an opening that
   states the claim and why it matters for a corporate reader in the first two
   sentences, then `##` section headings if needed. This is a daily briefing,
   not an essay: keep it skimmable in under a minute. Leave `TODO:` notes where
   evidence/examples are needed. Do NOT pad it out into a finished slop draft.
